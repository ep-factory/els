...
<script type="text/javascript">
  $(document).ready(function(){
    $('.sf_admin_form_field_end_hour input, .sf_admin_form_field_start_hour input').live('change', function(){
      ...
      $('.sf_admin_form_field_time_spent span').html(str_pad(Math.round($diff/(1000*60*60)), 2, 0, 'STR_PAD_LEFT') + 'h' + str_pad(Math.round(($diff%(1000*60*60))/(1000*60)), 2, 0, 'STR_PAD_LEFT'));
      $('.sf_admin_form_field_time_spent input:hidden').val($diff);
    });
