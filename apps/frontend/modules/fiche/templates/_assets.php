...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('.sf_admin_form_field_is_resolved input:radio').live('change', function(){
      if($(this).val() == 1) {
        $('#fiche_resolved_author_id').val(<?php echo $sf_user->getGuardUser() ? $sf_user->getGuardUser()->getPrimaryKey() : null ?>);
      }
    });
    $('.sf_admin_form_field_is_finished input:radio').live('change', function(){
      if($(this).val() == 1) {
        $('#fiche_finished_author_id').val(<?php echo $sf_user->getGuardUser() ? $sf_user->getGuardUser()->getPrimaryKey() : null ?>);
      }
    });
